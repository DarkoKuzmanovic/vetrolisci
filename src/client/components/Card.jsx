...
const Card = memo(
  ({
    ...
    // Compute motion props once at component top
    ...
    if (showBack) {
      ...
      return (
        <MotionDiv className={`card card-back ${isPlaced ? "placed" : ""} ${className}`} {...motionProps}>
          <LazyImage src={`/cards/backs/${backImagePath}`} alt="Card back" className="card-image" />
        </MotionDiv>
      );
    }

    if (!card) {
      return <MotionDiv className={`card card-empty ${className}`} {...motionProps}></MotionDiv>;
    }
