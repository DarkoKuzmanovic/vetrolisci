...
function calculateSymbolPoints(faceUpCards, breakdown) {
  ...
  breakdown.crosses = Math.abs(
    faceUpCards.filter((card) => card.scoring < 0).reduce((sum, card) => sum + card.scoring, 0),
  );
...
function calculateSpecialCardBonus(specialCard, faceUpCards) {
  ...
  if (specialCard.color === "multi") {
    ...
    return otherCards.filter((card) => card.color === specialCard.color || card.color === "multi").length;
  }
}
...
function calculateColorZoneBonus(grid, currentRound, breakdown) {
  ...
  const largestZone = colorZones.reduce(
    (largest, zone) => {
